...
structure and direct in mapping:
    ...
    <data-dir>/xmp/<library>/<trip>/*.xmp        # Photos-19/2019-01-13 Hill Park/
...
Usage:
    ./tool-audit                                  # report + worklists -> project/reports
    ./tool-audit --snapshot after-2024-recovery   # ...and archive/NN-...-after-2024-recovery.md
...
def render(result, data_dir, missing, unprocessed):
    ...
    w("")
    w(f"Generated by `tools/audit_dataset.py` over `{display_path(data_dir)}`. Read-only; the dataset is not")
    w("modified. Re-run with `./tool-audit`.")
    w("")
    w("## Layout")
    w("")
    w("```")
    w("<data-dir>/xmp/<library>/<trip>/*.xmp     # Photos-19/2019-01-13 Hill Park/_D8S0025.xmp")
