...
def days_off(rec):
    ...
    A month-precision folder (`2018-11 Highlands`) names a whole month, so anything
...
def render(groups, records, unparsable, keyless, misfiled, mislabelled, xmp_dir, threshold):
    ...
    w("")
    w(f"Generated by `tools/dedup_sidecars.py` over `{xmp_dir}`. Read-only -- nothing in the")
