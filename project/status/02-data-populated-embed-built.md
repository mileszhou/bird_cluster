...
populated on `localhost` and the embed client + shared libraries are written and exercised
...
- `code/lib/xmp_labels.py` — read categories + `py-cn-en(NN%)` labels from `dc:subject`. The
  ...
  but carry no percentage, and pre-existing user tags (`bhl-Hill Park`, `add=20231014`) coexist.
...
## Environment notes (localhost, GB10)
