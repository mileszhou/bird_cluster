...
`--llama-url` now default from `config.toml` instead of hardcoded `localhost`/`localhost` — `run-vllm`
...
reinvented. Next session runs directly on a host (`localhost`) with GPU + `data/` populated — start
