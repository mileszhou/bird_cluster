...
localhost), and the pipeline code that runs here (or wherever) is a thin non-GPU HTTP client.
...
designed (localhost-style GPU host running `embed_server.py`, non-GPU client elsewhere) since
...
separate from `.env` (secrets only, gitignored). `[servers.vllm]` (localhost:8000),
`[servers.embed]` (localhost:9100), `[servers.llama_cpp]` (localhost:8080) are already defined.
...
- CLI flags: `--model`, `--port`, `--host` — `--port`/`--host` default from `config.toml`
  `[servers.embed]` (currently `localhost:9100`), matching `embed.py`'s client-side default.
...
- Whether vLLM's own server (already running for the VLM backend) can serve DINOv3 as a
  ...
  dedicated FastAPI server instead. Worth a 5-minute check against localhost's vLLM version once
