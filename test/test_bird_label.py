...
def tree(tmp_path):
    ...
    for library, trips in {
        ...
        "Photos-24": ["2024-10-13 southport"],
    }.items():
        for trip in trips:
...
def test_keys_carry_the_folder_not_just_the_basename(tree):
    ...
    assert keys == {
        ...
        "Photos-24/2024-10-13 southport/_D8S0025.xmp",
    }
...
def test_user_keywords_survive_and_stay_first(tmp_path):
    p = sidecar(tmp_path, ["bhl-Hill Park", "bird", "old-旧-old bird(95%)"])
    bl.set_keywords_in_xmp(p, "bird", "new-新-new bird(88%)")
    assert subjects(p) == ("bhl-Hill Park", "bird", "new-新-new bird(88%)")
...
def test_writing_is_idempotent(tmp_path):
    ...
    p = sidecar(tmp_path, ["bhl-Hill Park"])
    for _ in range(3):
        bl.set_keywords_in_xmp(p, "bird", "new-新-new bird(88%)")
    assert subjects(p) == ("bhl-Hill Park", "bird", "new-新-new bird(88%)")
