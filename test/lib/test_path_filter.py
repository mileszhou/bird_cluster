...
def test_a_line_may_be_any_depth():
    ...
    nested = "Photos-24/2024-05-01 Coastal Park/2024-04-19 Rivertown Birds"
    key = f"{nested}/_D5D0372.jpg"
    assert not PathFilter(exclude=[nested]).allows(key)
    # ...and a parent line takes the nested child with it
    assert not PathFilter(exclude=["Photos-24/2024-05-01 Coastal Park"]).allows(key)
...
def test_read_paths_tolerates_a_spreadsheet_bom(tmp_path):
    ...
    p = tmp_path / "list.txt"
    p.write_text("Photos-19/2019-01-13 Hill Park\n", encoding="utf-8-sig")
    assert read_paths(p) == ["Photos-19/2019-01-13 Hill Park"]
