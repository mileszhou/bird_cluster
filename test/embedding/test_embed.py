...
from code.embedding.embed import already_done, collect, year_dirs
from code.lib.jpg_index import MatchPolicy
...
def sidecar(data_dir, year, half_year, trip, stem, keywords):
    d = data_dir / "xmp" / f"result-{year}" / "raw" / half_year / trip
    d.mkdir(parents=True, exist_ok=True)
    items = "".join(f"<rdf:li>{k}</rdf:li>" for k in keywords)
    (d / f"{stem}.xmp").write_text(XMP.format(items=items), encoding="utf-8")


def jpeg(data_dir, half_year, stem):
    d = data_dir / "jpg" / half_year
    d.mkdir(parents=True, exist_ok=True)
...
def data_dir(tmp_path):
    ...
    d = tmp_path / "data"
    sidecar(d, "2019", "2019.1", "trip a", "bird_ok", ["bird", "hj-黑鹤-black stork(98%)"])
    jpeg(d, "2019.1", "bird_ok")
    sidecar(d, "2019", "2019.1", "trip a", "bird_nojpg", ["bird", "ml-麻雀-house sparrow(95%)"])
    sidecar(d, "2019", "2019.1", "trip a", "scenic", ["scenery", "x-y-A hill(98%)"])
    jpeg(d, "2019.1", "scenic")
    sidecar(d, "2021", "2021.3", "trip b", "bird_2021", ["bird", "hj-黑鹤-black stork(90%)"])
    jpeg(d, "2021.3", "bird_2021")
    return d


def test_year_dirs_filters(data_dir):
    assert [y for y, _ in year_dirs(data_dir, None)] == ["2019", "2021"]
    assert [y for y, _ in year_dirs(data_dir, ["2019"])] == ["2019"]
    assert year_dirs(data_dir, ["1999"]) == []


def test_collect_keeps_only_resolvable_birds(data_dir):
    cands, stats, per_year = collect(data_dir, ["2019"], MatchPolicy.SAME_YEAR, 0.0)
...
def test_collect_records_provenance(data_dir):
    cands, _, _ = collect(data_dir, ["2019"], MatchPolicy.SAME_YEAR, 0.0)
    c = cands[0]
    assert c.key == "2019/2019.1/trip a/bird_ok.xmp"
    assert (c.year, c.half_year, c.trip) == ("2019", "2019.1", "trip a")
...
def test_collect_year_filter_spans_years(data_dir):
    cands, _, per_year = collect(data_dir, None, MatchPolicy.SAME_YEAR, 0.0)
    assert per_year == {"2019": 1, "2021": 1}
    # Keys are year-scoped, so identical stems in different years cannot collide.
    assert len({c.key for c in cands}) == 2


def test_min_confidence_drops_low_labels(data_dir):
    cands, stats, _ = collect(data_dir, None, MatchPolicy.SAME_YEAR, 0.95)
    assert [c.stem for c in cands] == ["bird_ok"]
