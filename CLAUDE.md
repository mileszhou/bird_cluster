...
a value that works for a stranger**, so the hosts say `localhost`; they used to say `localhost`
and `localhost`, which meant a public clone either failed to resolve them or reached somebody
...
```
data/xmp/<Photos-YY>/<trip>/*.xmp     # Photos-19/2019-01-13 Hill Park/_D8S0025.xmp
...
- **`bird` was protected** so a fresh false negative could not drop a photo from the clustering
  ...
  era — a wombat carrying `common myna` because a Harbour City photo with the same stem
