...
- `--batch-size INT` — images in flight at once (default 1). Applies to **`vllm` and `openai` alike**, since the batch path is a thread pool of ordinary HTTP requests rather than a vLLM feature: a local server turns the concurrency into continuous batching, a hosted API into parallel calls. Restricting it to `vllm` was historical and cost the cloud backend an order of magnitude — 26,104 images at 2.2s serial is sixteen hours and about two at eight in flight. Against a metered endpoint concurrency is what provokes a 429, so raise it deliberately; the transient retry handles the rate limits that follow, it does not make a reckless value safe. `llama.cpp` keeps its own predictor and stays serial
...
- `--sample-seed INT` — process images in a reproducible random order rather than walk order, **with or without `--limit`**, so that any prefix of a run is an unbiased sample. Walk order is *path* order, so a trial takes a handful of consecutive trips: 2,000 bird images that way is **43 folders and 326 species**, against **415 and 778** drawn at random. Anything fitted on the first — a confidence threshold, a cost estimate, an error rate — describes those trips rather than the library. The same trap CLAUDE.md already records for the embedding JSONL in `predict_taxa`, which reads ~0.45 on the head of the file where the true figure is 0.29. Tying it to `--limit` was the first mistake: a full run in walk order is fine only if it finishes, and a run interrupted or inspected early leaves a path-ordered prefix — the same bias. The seed lands in `args.json`; note a *resumed* run draws from what is left, so reproducing an original draw wants a fresh `--output-dir`
...
- `args.json` — CLI arguments for reproducibility. **The previous set is kept, not overwritten**: every other artifact here accumulates (CSV, checkpoint, log are appended), so a directory can hold rows from several runs, and args.json alone described only the last — losing the parameters that produced most of the rows. A differing set moves the old one to `args.N.json` and says which keys changed; an identical one is left alone. Resuming with a different model or scope is legitimate and dangerous to do silently, since the appended rows are not comparable to the ones already there
...
- **`label_cn` is normalised to simplified Chinese** at the boundary (`to_simplified()`), because a model asked for "the standard Chinese name" answers in whichever script it likes: 普通翠鳥 against 普通翠鸟 is one bird under two keywords, so a photo manager lists it twice and any join on the label splits it. Simplified because the existing labelling already is — 1,509 of 49,224 rows were not. **It is not the main cause of unstable names**: of the 980 English names carrying more than one Chinese form, simplification collapses 7, and the other 973 differ by genuine synonymy — two real Chinese names for one bird — which no script conversion touches. That remains open, and affects ~11,153 images
- **`label_sci` is the binomial, and it is asked for because it is the checklist's strong key.** Added 2026-09-04. 19.6% of bird images fell back to BioCLIP's pixel guess for their taxonomy — not because the labels were odd but because `great crested grebe`, `eurasian hoopoe`, `white wagtail` and `common moorhen` have no entry in TreeOfLife's *common-name* index while every one is present by binomial (10,788 common names against 11,131 taxa, and the gap is exactly ordinary birding vocabulary). `map_label_taxa` matches on the binomial first and falls back to the name, so a run without the column behaves exactly as before. `normalise_binomial()` checks shape only — two Latin words, genus capitalised — and existence is checked against the checklist, which is what makes a wrong binomial cheap: it matches nothing and falls through, where a wrong *common* name has no backstop. **This is a CSV schema change**, so `check_csv_schema()` refuses to resume an older run's CSV; that is the guard working, and a fresh `--output-dir` is the answer
