...
`duplicate_frames.csv` carries only the frames needing a decision (240 of 6,060); the ordinary
wraparounds are ~95% of the collisions and are never acted on, so listing them buried the rows
that matter. `./run-audit --all-collisions` writes the full set to `duplicate_frames_all.csv`
when the question is whether a given collision was examined at all.
