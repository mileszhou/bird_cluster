...
./run-cpp                         # llama.cpp server at http://localhost:8080
./run-vllm                        # vLLM server at http://localhost:8000/v1 (recommended)
./run-tf                          # HuggingFace Transformers

# Or directly with options:
python3 -m code.bird_label --approach vllm --vllm-url http://localhost:8000/v1 --model "Qwen/Qwen3-VL-132B-Instruct" --conf-threshold 0.6
```

Key CLI flags:
- `--approach` — `chatgpt`, `llama.cpp`, `vllm`, or `transformer`
- `--vllm-url URL` — vLLM OpenAI-compatible server endpoint (default `http://localhost:8000/v1`, vllm approach only)
...
`run-vllm` is the recommended approach — it talks to a vLLM OpenAI-compatible server (`--vllm-url`, default `http://localhost:8000/v1`) rather than loading the model in-process, so start the server separately (see `run-server-vllm` for an example) before running this. On startup the script probes `{vllm-url}/models` and swaps in whatever model the server actually has loaded if `--model` doesn't match, so the requested model name doesn't need to be exact.
